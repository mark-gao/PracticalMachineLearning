...
```
Result of acceleration features
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
```
Result of magnetometer features
```{r, echo=FALSE, message=FALSE, warning=FALSE}
confusionMatrix(predRF, val1$classe)
```
...
confusionMatrix(predBAG, val1$classe)
```
LDA output:
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
confusionMatrix(predLDA, val1$classe)
